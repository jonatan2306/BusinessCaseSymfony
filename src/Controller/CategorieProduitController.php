...
{
    // la je récupère toutes les données (toutes les catégories du produit)
    ...
    // la je récupère et je post la donnée
    ...
    {
        // là je créer un nouvelle objet categorie produit
        $categorieProduit = new CategorieProduit();
        $form = $this->createForm(CategorieProduitType::class, $categorieProduit);
        $form->handleRequest($request);
        
        if ($form->isSubmitted() && $form->isValid()) {
            $categorieProduitRepository->add($categorieProduit, true);
